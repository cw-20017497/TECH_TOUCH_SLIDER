...
__interrupt static void r_uart1_interrupt_receive(void)
{
    ...
    if( err_type == 0 )
    {
        if( HAL_IsFullRecvBuffer( COMM_ID_TOF ) == FALSE )
        {
            HAL_SetRecvBuffer( COMM_ID_TOF, rx_data );
        }
        else
        {
            HAL_InitCommId( COMM_ID_TOF );
    ...
    StartTimer( TIMER_ID_COMM_TOF_RX, UART1_RX_TIME_STAMP );
}

__interrupt static void r_uart1_interrupt_send(void)
{
    if( IsCompleteTx( COMM_ID_TOF ) == FALSE )
    {
        HAL_SendByte( COMM_ID_TOF );
    }
    else
    {
        HAL_InitCommId( COMM_ID_TOF );
